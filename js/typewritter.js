...
(function () {
    ...
    for (var a = 0, c = ['ms', 'moz', 'webkit', 'o'], b = 0; b < c.length && !window.requestAnimationFrame; ++b)
      (window.requestAnimationFrame = window[c[b] + 'RequestAnimationFrame']),
        (window.cancelAnimationFrame =
          window[c[b] + 'CancelAnimationFrame'] || window[c[b] + 'CancelRequestAnimationFrame']);
    ...
    if (!this._settings.eventRunning && 0 >= this._settings.eventQue.length) {
      ...
      setTimeout(function () {
        c.options.loop &&
          ...
          4 < c.eventLoopRerun
            ? (console.error('Maximum amount of loop retries reached.'), c._stopEventLoop())
            : (c.options.devMode && console.log('Looping events.'), c._rerunCalledEvents()));
      }, 1e3);
    }
  };
  ...
  b._typingAnimationFrame = function (a) {
    ...
    if (0 == c._settings.stringToTypeHTMLArray.length)
      return window.cancelAnimationFrame(c._typingAnimation), this._resetEventLoop('typingAnimationFrame'), !0;
  ...
  b._getStringsAsCharsArray = function () {
    ...
    if (!(this.options.strings instanceof Array))
      return a
        ...
        : console.error('Typewriter only accepts strings or an array of strings as the input.');
    ...
    if (!this._settings.cursorAnimationPaused) {
      ...
      1 == this._settings.opacityIncreasing &&
        (1 <= this._settings.currentOpacity &&
          ((this._settings.opacityIncreasing = !1), (this._settings.currentOpacity = 1)),
        (this._settings.currentOpacity += a));
      0 == this._settings.opacityIncreasing &&
        (0 >= this._settings.currentOpacity &&
          ((this._settings.opacityIncreasing = !0), (this._settings.currentOpacity = 0)),
        (this._settings.currentOpacity -= a));
  ...
  b._pauseCursorAnimation = function () {
    this._settings.cursorAnimationPaused ||
      (window.cancelAnimationFrame(this._cursorAnimation), (this._settings.cursorAnimationPaused = !0));
  };
